...
export class AuthServices {
    ...
    async LoginUser(content: { loginOrEmail:string, password:string, ip: string, title:string }){
        const foundUser = await this.usersRepo.ShowUser(content.loginOrEmail)
        ...
        {throw new InvalidCredentialsError('Wrong Credentials', [{message:'Wrong Login or email', field:'loginOrEmail' }]);}
            ...
            const accessToken= await this.jwtService.createToken(foundUser._id ,deviceId)
    ...
    async refreshAccessToken(rToken: string){
        const payload  = await this.jwtService.verifyRToken(rToken)  //ошибка валидации выбрасывается в jwtService

        const foundDevice = await this.devicesRepo.ShowDevice(payload.deviceId)
        ...
        {throw new InvalidCredentialsError('Token is not valid',
            ...
            const newRToken = await this.jwtService.createRToken(payload.userId, payload.deviceId)
            const newAToken = await this.jwtService.createToken(payload.userId, payload.deviceId)
    ...
    async LogoutUser(rToken:string){
        const payload:RefreshTokenPayloadType  = await this.jwtService.verifyRToken(rToken)
        ...
        const res = await this.devicesRepo.DeleteDevice(foundDevice._id.toString())
    ...
    async passwordRecovery(email: string){
        ...
        const foundUser = await this.usersRepo.ShowUser(email)
        if (!foundUser)
        {return}
        ...
        await this.usersRepo.UpdateUser(foundUser)  //в принципе можно не проверять на ошибку обновления(наверное)
    ...
    async newPassword(content:InputNewPassType){
        ...
        const foundUser = await this.usersRepo.ShowUser(content.recoveryCode) //TODO проверить, что поиск адаптирован
        ...
        const res = await this.usersRepo.UpdateUser(foundUser)  // обновляем юзера
