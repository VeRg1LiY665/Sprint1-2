...
export const blogsController= {
    getBlogs: async (req: Request, res: Response) => {
        ...
        const result = BlogsQRepo.PaginationMap({pageNumber, pageSize, blogs})
    ...
    (!posts) ? res.status(404).json('Posts for given blog not found') : res.status(200).json(PostsQRepo.PaginationMap({pageNumber, pageSize, posts}))}
    },
